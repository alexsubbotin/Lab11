...
{
    // Republic class (inherits AbstrState class)
    class Republic : AbstrState, ICloneable
    {
        ...
        // Shallow copy
        public Republic ShallowCopy()
        {
            return (Republic)this.MemberwiseClone();
        }

        // Clone
        public object Clone()
        {
            return new Republic("Clone " + this.Name, this.LeaderName, this.Population, this.Age, this.Continent,
                this.PresidentTerm, this.ParlamentMembersNumber, this.ParlamentTerm);
        }
    }
}
